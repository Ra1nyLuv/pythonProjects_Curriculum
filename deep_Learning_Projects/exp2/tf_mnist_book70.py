# 下载和导入MNIST数据集。它会自动创建一个'MNIST_data'的目录来存储数据。mnist是一个轻量级的类。它以Numpy数组的形式存储着训练、校验和测试数据集。同时提供了一个函数，用于在迭代中获得minibatch，
...
# 训练和评估模型
# 这个模型的效果如何呢？
print(x_image,h_conv1,h_pool1,h_conv2,h_pool2,h_fc1_drop,y_conv)
...
with tf.Session() as sess:
  ...
  writer=tf.summary.FileWriter(r'C:\Users\sophia\Desktop\tmp',sess.graph)
  writer.close()
